...
class SimpleIPTVRepository(context: Context) {
    private val playlistPrefs: SharedPreferences = context.getSharedPreferences("saved_playlists", Context.MODE_PRIVATE)
    private val activePlaylistKey = "active_playlist_id"

    /**
     * Save a new playlist config (M3U or Xtream) to preferences.
     */
    ...
    /**
     * Get all saved playlists from preferences.
     */
    ...
    private val prefs: SharedPreferences = context.getSharedPreferences("xtream_configs", Context.MODE_PRIVATE)
    private val playlistCache: SharedPreferences = context.getSharedPreferences("playlist_cache", Context.MODE_PRIVATE)
    ...
    suspend fun loadChannelsFromM3U(url: String): List<Channel> = withContext(Dispatchers.IO) {
        try {
    ...
    suspend fun loadChannelsFromXtream(config: XtreamConfig, forceRefresh: Boolean = false): List<Channel> = withContext(Dispatchers.IO) {
        try {
           
            if (!forceRefresh) {
               
                val currentTime = System.currentTimeMillis()
                if (memoryCache != null && (currentTime - memoryCacheTime) < memoryCacheValidityMs) {
                    println("DEBUG: Returning from memory cache (${memoryCache!!.size} channels)")
                    _cachedChannels.value = memoryCache!!
                    return@withContext memoryCache!!
                }

               
                val cachedChannels = loadPlaylistFromCache("xtream_${config.host}_${config.username}")
                if (cachedChannels.isNotEmpty()) {
                    println("DEBUG: Returning from persistent cache (${cachedChannels.size} channels)")
            ...
            val categoriesResponse = service.getLiveCategories(config.username, config.password)
            val categoryMap = if (categoriesResponse.isSuccessful && categoriesResponse.body() != null) {
                categoriesResponse.body()!!.associate { it.category_id to it.category_name }
            } else {
                emptyMap()
            }
            ...
            val channels = xtreamChannels.map { ch ->
                val categoryName = categoryMap[ch.category_id] ?: "Uncategorized"
                val host = base.trimEnd('/')
                Channel(
                    name = ch.name,
                    url = "$host/live/${config.username}/${config.password}/${ch.stream_id}.m3u8",
                    logo = ch.stream_icon,
                    group = categoryName
                )
            }
    ...
    suspend fun testXtreamConnection(host: String, username: String, password: String): Result<Boolean> {
        return try {
